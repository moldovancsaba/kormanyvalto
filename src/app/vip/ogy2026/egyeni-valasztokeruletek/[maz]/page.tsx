...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (countyConstituencies.length === 0) {
    return buildPageMetadata({
      title: `VIP Szavazás 2026 - ${maz}`,
      description: "A keresett vármegyei lista nem érhető el.",
      path: `/vip/ogy2026/egyeni-valasztokeruletek/${maz}`,
    });
  }

  return buildPageMetadata({
    title: `VIP Szavazás 2026 - ${countyConstituencies[0].mazNev}`,
    description: `VIP - ${countyConstituencies[0].mazNev}`,
    path: `/vip/ogy2026/egyeni-valasztokeruletek/${maz}`,
  });
}

export default async function VipCountyPage({ params }: PageProps) {
  const { maz } = await params;
  const countyConstituencies = getConstituenciesByCounty(maz);
  ...
  return (
    <PageShell
      navItems={getVipNavItems("/vip/ogy2026/egyeni-valasztokeruletek", [
        { href: "/vip/ogy2026/egyeni-valasztokeruletek", label: "Vissza a listához" },
      ])}
    >
      <PageIntro
        eyebrow="VIP EVK 2026"
        title={countyName}
        intro="2. lépés: válassz egyéni választókerületet. Innen közvetlenül a VIP szavazáshoz jutsz."
      />

      <section className="context-panel" aria-label="Vármegyei navigációs összefoglaló">
        <p className="context-panel-eyebrow">VIP vármegyei nézet</p>
      ...
      <section className="button-list" aria-label="Körzetek listája">
        {countyConstituencies.map((c) => {
          ...
          return (
            <Link
              key={`${c.maz}-${c.evk}`}
              href={`/vip/ogy2026/egyeni-valasztokeruletek/${c.maz}/${c.evk}`}
